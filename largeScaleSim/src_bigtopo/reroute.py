...
def get_route_alg1 (file_path):
        ...
        for randome_time in range(0,1):
            for entries in range(31,32):             #300-4200
                ...
                for alg in range(0,1):
                    ...
                    if alg == 2:  #启发式算法 ,ospf need entries
                        used_ce = defaultdict(lambda:None)
    ...
    '''            
                    if alg == 4:  #启发式算法(OSPF do not need entries )
                        used_ce = defaultdict(lambda:None)
                        used_entries  = defaultdict(lambda:None)
                        for v in all_node:
                            used_entries[v] = 0
                        ...
                        for i in range(0,len(source_random)):
                            ...
                            if temp1 == 10000: #表示几条可行路径均不满足流表约束，此时直接ospf，  那么只需要使用链路容量
                                ...
                                for n in range(0,len(all_node_alone[3*i+k])):
                                    used_entries[all_node_alone[3*i+k][n]] = used_entries[all_node_alone[3*i+k][n]] + 1
                            #print lambda1,all_edge_alone[3*i+k]     
                        print "greedy",lambda1,temp2  
                        p1 = file_path+"/alg4.json"
                        fd = open(p1, 'a')
                        fd.write(json.dumps(lambda1))
                        fd.write(json.dumps("/"))
                        fd.close()     
        ...
        p1 = file_path+"/avg_alg0.json"  #rounding
        fd = open(p1, 'a')
        fd.write(json.dumps(float(alg0)/30))
        fd.write(json.dumps("/"))
        fd.close()  
        
        p1 = file_path+"/avg_alg1.json"  #rounding
        fd = open(p1, 'a')
        fd.write(json.dumps(float(alg1)/30))
        fd.write(json.dumps("/"))
        fd.close()  
        
        p1 = file_path+"/avg_alg2.json"  #rounding
        fd = open(p1, 'a')
        fd.write(json.dumps(float(alg2)/30))
        fd.write(json.dumps("/"))
        fd.close()  
        
        p1 = file_path+"/avg_alg3.json"  #rounding
        fd = open(p1, 'a')
