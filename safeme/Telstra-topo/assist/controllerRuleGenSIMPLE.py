...
for i in range(1, 18):
    sfile.append("../runtime-simple/s"+str(i)+"-runtime.json")
...
for i in range(1, 18):
    ...
    config[sw]["p4info"] = "build/simple.p4.p4info.txt"
    config[sw]["bmv2_json"] = "build/simple.json"
...
# for SIMPLE, first we load SFC rules from json file
...
for route in routes:
    ...
    startNode = route[0]
    endNode = route[l-1]
    # build another route dict for SIMPLE to use
...
nf_conf_file = open("../runtime-simple/NFmapping.json", "r")
nf_conf = json.load(nf_conf_file)
nf_conf_file.close()

sfc_file = open("../runtime-simple/sfcRules.json", "r")
...
# then we build SIMPLE rules based on INPORT to identify the path through NFs
for rule in sfc_rules:
    ...
    for i in range(0, nodeNum-1):
        ...
        if endNode[0:1] == "h": # means the last segment ends with dst host from last NF, we no need deploy rules for host, so end index should stop at plen-2
            endIndex = plen - 2
        # do rules generation for inport based SIMPLE routing
