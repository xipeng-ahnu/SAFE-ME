...
def program_switch(addr, device_id, sw_conf_file, workdir, proto_dump_fpath):
    ...
    try:
        ...
        if 'table_entries' in sw_conf:
            ...
            entryNum = len(table_entries)
            info("Inserting %d table entries..." % entryNum)
            startTime = time.time()
            for entry in table_entries:
                info(tableEntryToString(entry))
                insertTableEntry(sw, entry, p4info_helper)
            endTime = time.time()
            info("Entries: %d , Total Time: %f ms, AVERAGE: %f ms" % (entryNum, (endTime - startTime) * 1000, (endTime - startTime) * 1000 * 1000 / entryNum / 1000))
